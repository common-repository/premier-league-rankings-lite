<?php
/*
Plugin Name: Premier League Football Rankings Lite
...
function PREMLEAGUESPRLPageURL() {
 $PREMLEAGUESPRLpageURL = 'http';
 $PREMLEAGUESPRLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $PREMLEAGUESPRLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $PREMLEAGUESPRLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $PREMLEAGUESPRLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_premleaguesprlstats() 
{
?>
<h2>Premier League Rankings Lite</h2>
<?php premleaguesprl_stats(); ?>
<?php
}

function premleaguesprlstats_install()
{
register_sidebar_widget(__('Premier League Rankings Lite'), 'widget_premleaguesprlstats'); 
}
add_action("plugins_loaded", "premleaguesprlstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'premleaguesprl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'premleaguesprl_stats_remove' );

function premleaguesprl_stats_install() 
{
...
add_option("premleaguesprl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("premleaguesprl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("premleaguesprl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE PREMLEAGUE Power Rankings Installation", "Hi\n\nLITE PREMLEAGUE Power Rankings Activated at \n\n".PREMLEAGUESPRLPageURL()."\n\nPREMLEAGUE Power Rankings Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function premleaguesprl_stats_remove() 
{
/* Deletes the database field */
delete_option('premleaguesprl_scroll_text_color');
delete_option('premleaguesprl_scroll_text_color1');
delete_option('premleaguesprl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'premleaguesprl_stats_admin_menu');

function premleaguesprl_stats_admin_menu() {

add_options_page('Premier League Rankings Lite', 'Premier League Rankings Lite Settings', 'administrator', 'premleague-power-rankings-lite.php', 'premleaguesprl_stats_plugin_page');
}
}
function premleaguesprl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('premleaguesprl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('premleaguesprl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('premleaguesprl_scroll_text_color2'));

$myads = "http://www.ibet.ws/prem_stats_rankings_magpie_lite/prem_stats_rankings_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $myads;
$rss = premleaguesprl_fetch_rss( $url );
...
function premleaguesprl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>Premier League Rankings Options Page</h2>
...
<p>
<?php echo premleaguesprl_stats(); ?>
</p>
<?php echo $myads; ?>
<!-- Start Advanced Plugins List -->
  <h2>If You Want MORE Stats and Information:</h2>
  <p>A93D Offers FREE upgrades for this stats package, that allow you to display advanced and more complete PREMLEAGUE Power Rankings.
  ...
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/premier-league-rankings.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/premier-league-rankings.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
